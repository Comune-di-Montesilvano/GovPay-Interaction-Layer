...
class EntrateRepository
{
    ...
    * @param array{idEntrata?:string,tipoEntrata?:array,ibanAccredito?:string,codiceContabilita?:string,tipoBollo?:string,abilitato?:bool} $e
    ...
    $ibanAppoggio = $e['ibanAppoggio'] ?? null;
    if ($ibanAppoggio === '-') {
        $ibanAppoggio = null;
    }
    ...
    $now = date('Y-m-d H:i:s');
        ...
        $sql = 'INSERT INTO entrate_tipologie (id_dominio, id_entrata, descrizione, iban_accredito, iban_appoggio, codice_contabilita, tipo_bollo, tipo_contabilita, abilitato_backoffice, sorgente, created_at, updated_at)
        VALUES (:id_dominio, :id_entrata, :descrizione, :iban, :iban_appoggio, :codice, :tipo_bollo, :tipo_contabilita, :bo, "backoffice", :now_created, :now_updated)
        ON DUPLICATE KEY UPDATE descrizione = VALUES(descrizione), iban_accredito = VALUES(iban_accredito), iban_appoggio = VALUES(iban_appoggio), codice_contabilita = VALUES(codice_contabilita), tipo_bollo = VALUES(tipo_bollo), tipo_contabilita = VALUES(tipo_contabilita), abilitato_backoffice = VALUES(abilitato_backoffice), updated_at = VALUES(updated_at)';
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([
    ...
    /** Aggiorna l'IBAN appoggio della tipologia. */
    public function updateIbanAppoggio(string $idDominio, string $idEntrata, ?string $iban): void
    {
        $sql = 'UPDATE entrate_tipologie SET iban_appoggio = :iban, updated_at = :now WHERE id_dominio = :dom AND id_entrata = :ent';
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([
            ':iban' => ($iban === null || trim($iban) === '') ? null : trim($iban),
            ':now' => date('Y-m-d H:i:s'),
