...
- **Gotcha `main` non protetto**: nessun branch protection su GitHub (`GET .../branches/main/protection` → 404) — niente forza PR/status check/no-force-push prima del merge. Best practice valutate ma non ancora implementate (agosto 2026): branch protection su `main`, pin delle action a commit SHA (tag mutabili es. `@v6` sono supply-chain risk), PHPStan in CI (Semgrep copre solo pattern di sicurezza, non bug di tipo/logica), CODEOWNERS su path sensibili (`app/Security/`, `backoffice/src/routes/`, `migrations/`).
