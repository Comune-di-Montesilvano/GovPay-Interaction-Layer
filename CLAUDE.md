...
# Path assoluti Linux (/var/www/html/...) passati a docker exec/docker cp in git-bash vengono
# riscritti (es. in C:/Program Files/Git/var/www/html/...) e il comando fallisce silenziosamente
# ("No such file"/"Could not open input file") — sempre MSYS_NO_PATHCONV=1 davanti al comando,
# non solo per composer.
...
- **`gh pr merge` con branch "behind" la base**: fallisce con "head branch is not up to date". Fix: `gh api -X PUT repos/<org>/<repo>/pulls/<N>/update-branch`, poi poll `gh pr view <N> --json mergeStateStatus` finché non torna `CLEAN` (passa per `BLOCKED` mentre CI rigira sul nuovo commit) prima di ritentare il merge.
...
- **Rendicontazione GovPay `iuv` vs `numeroAvviso`**: bridge legacy (Geri/Dilazione) indicizza sul numero avviso completo (aux digit + IUV, 18 cifre, es. `300607000007021733`), non sullo IUV corto GovPay (17 cifre, es. `00607000007021733`). `RendicontazioneEngineService` paddda prima di `LegacyRendicontazioneBridgeClient::invia()` — non rimuovere il padding, causa "Rata non trovata" silenzioso lato bridge (mai su Sentry, `esito=false` inghiottito in `markErrore`). Lookup GovPay diretto by avviso: `GET /pendenze/byAvviso/{idDominio}/{numeroAvviso}` (`PendenzeController::fetchPendenzaByAvvisoRaw`).
- **Gap routing rendicontazione senza `ERRORE`**: se `RendicontazioneRouter::decide()` non matcha GERI/DILAZIONE per una pendenza che dovrebbe, il motore la marca `GESTITO` nel ramo auto (`RendicontazioneEngineService::processaRigaSpecifica`) SENZA mai chiamare il bridge — nessuna riga `ERRORE`, nessuna traccia. Se un pagamento risulta incassato su GovPay ma non registrato lato gestionale esterno, verificare qui prima di fidarsi dello stato `GESTITO` in `flussi_rendicontazioni`.
- **`scripts/legacy-bridge/rendicontazione_bridge.php` nel repo è solo riferimento**: il file realmente deployato (`servizi.comune.montesilvano.pe.it`, path tipico `/pagopa/gestionale/rendicontazione_bridge.php`) può divergere — copiato a mano, nessuna pipeline di sync. Caso reale: `Authorization` header non arrivava a `$_SERVER['HTTP_AUTHORIZATION']` per config Apache/PHP-CGI del server legacy (401 anche con token corretto), servito fallback `getallheaders()`. Se tocchi questo file, applica il fix su entrambe le copie o il repo diventa fuorviante.
